...
**文字化け**:
opencodeがPowerShellを介してコマンドを実行した後、コンソールの出力が文字化けする。具体的には `�����ꏊ` のような読めない文字列がセッションログの各所に記録された。opencodeのbash実行がPowerShellのエンコーディング設定に影響を与えている可能性がある。
...
`npx @opencode-ai/cli --version` は試せていないが、postCreateCommandで追加したエイリアスやラッパースクリプトが機能しなかったことは確かである。
...
postCreateCommand（コンテナ起動後に実行）ではなく、Dockerfile（イメージビルド時に実行）でopencodeをインストールする方式。postCreateCommandはコンテナ起動後に非同期的に実行されるため、タイミングによってはコマンドが使えない状態でユーザーが操作を始めてしまう。Dockerfile方式にすればイメージビルド時にインストールが完了するため、この問題は原理的に発生しない。ただし、最初のビルドに時間がかかる。
