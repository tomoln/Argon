...
description: "AWSサーバーレス（API Gateway + Lambda + DynamoDB）で個人メモツール「mycow2」を構築した記録。日付パーサー、CLIとDiscord Botの両対応、ed25519署名検証のハマりポイントを解説。"
tldr: "自分用のメモ＆スケジュールツールをAWSサーバーレスでゼロから構築。API Gateway + Lambda + DynamoDBのシンプル構成で月額0〜1USD。CLI（commander.js）とDiscord Botの両方から操作可能。ed25519署名検証で地獄を見た話も含む。"
---
...
**Notion**: 多機能で便利ですが、無料プランだとすぐワークスペースがいっぱいになってしまいます。継続して使い続けるには課金が必要になります。

**Obsidian**: ローカル志向で軽快に動作しますが、複数端末での同期は公式サービスの有料プランか、自分でクラウドストレージを運用する必要があります。

どちらも優れたツールではありますが、「Discordに投げるだけ」「CLIで一発」でメモが残って、かつスケジュール管理もできて、運用コストが月額数百円以下のツールが欲しくなりました。要件に合うものが見つからなかったので、自分で作ることにしました。
...
```
 +-----------+     +-------------+     +-----------+  
 |CLI (mycow)|----->| API Gateway |----->|API Lambda |
 |command.js |     | HTTP API    |     |CRUD+Search|  
 +-----------+     | /discord    |     +-----+-----+  
                   |             |           |        
 +-----------+     |             |     +-----+-----+  
 |Discord Bot|----->|             |----->|Disc Lambda|
 |Slash Cmd  |     +-------------+     |Cmd Handler|  
 +-----------+                       +-----+-----+    
                                           |          
                                           v          
                                     +-----------+    
...
このラッパー関数で、Discordのインタラクション形式とAPI GatewayのHTTPレスポンス形式を橋渡ししています。
